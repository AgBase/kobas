...
#PULL IN DATABASE FILES
if [[ -e seq_pep/$species'.pep.fasta' && -e sqlite3/$species'.db' && -e sqlite3/organism.db ]]
then
    echo "all files and dirs  present"
...
then
	...
	chmod a+rwx seq_pep/$species* sqlite3/$species*
        blastp -query $infile -db seq_pep/$species'.pep.fasta' -out $species'.tsv' -outfmt 6 $BLARGS
        annotate.py  -i $species'.tsv' -t blastout:tab -s $species -o $out $ARGS
    elif [[ "$intype" = "fasta:nuc" ]]
    then
        makeblastdb -in seq_pep/$species'.pep.fasta'  -parse_seqids -dbtype prot -out seq_pep/$species'.pep.fasta'
	chmod a+rwx seq_pep/$species* sqlite3/$species*
        blastx -query $infile -db seq_pep/$species'.pep.fasta' -out $species'.tsv' -outfmt 6 $BLARGS
        annotate.py -i $species'.tsv' -t blastout:tab -s $species -o $out'.tsv' $ARGS
    else
        annotate.py -i $infile -t $intype  -s $species -o $out'.tsv' $ARGS
   fi
#    sed -ni '/\#\#/,$p' $out
fi
...
then
    ...
    if [ -n "${blastdb}" ]; then ARGS="$ARGS -y $blastdb"; fi
     ...
     identify.py -f $fgfile -b $bgfile -o $out'_identify.txt' $ARGS
fi
...
then
	...
	chmod a+rwx seq_pep/$species* sqlite3/$species*
        blastp -query $infile -db seq_pep/$species'.pep.fasta' -out $species'.tsv' -outfmt 6 $BLARGS
        annotate.py  -i $species'.tsv' -t blastout:tab -s $species -o $out'_annotate.txt' $ARGS
    elif [[ "$intype" = "fasta:nuc" ]]
    then
        makeblastdb -in seq_pep/$species'.pep.fasta'  -parse_seqids -dbtype prot -out seq_pep/$species'.pep.fasta'
	chmod a+rwx seq_pep/$species* sqlite3/$species*
        ...
        annotate.py -i $infile -t $intype  -s $species -o $out'_annotate.txt'  $ARGS
    fi
    ...
    if [ -n "${blastx}" ]; then ARGS="$ARGS -x $blastx"; fi
    if [ -n "${blastdb}" ]; then ARGS="$ARGS -y $blastdb"; fi
    identify.py -f $fgfile -o $out'_identify.txt' $ARGS
